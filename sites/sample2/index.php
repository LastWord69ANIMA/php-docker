...
//テーブル作成(掲示板画面)
$sql = "CREATE TABLE IF NOT EXISTS  sample2Table"
. " ("
...
if (!empty($_POST["name"]) && !empty($_POST["comment"]) && !empty($_POST["pass"])) {
    ...
    foreach ($results as $row) {
        ...
        if (password_verify($pass, $CorrectPass)) {
            $sql = "INSERT INTO sample2Table (name, comment, image_file, audio_file, date, password) VALUES (:name, :comment, :image, :audio, :date, :password)";
...
elseif(!empty($_POST["delete"]) && !empty($_POST["del_pass"])){
    ...
    //以下、passがあっていれば実行
    $sql = 'SELECT * FROM sample2Table WHERE id=:id';
    $stmt = $pdo->prepare($sql);
    ...
    if($row && password_verify($del_pass, $row['password'])){
        $sql = 'DELETE FROM sample2Table WHERE id=:id';
        $stmt = $pdo->prepare($sql);
        $stmt->bindParam(':id', $delete, PDO::PARAM_INT);
        $stmt->execute();
    } else {
...
if( !empty( $_POST["edit_num"] )&& !empty( $_POST["edit_name"] )&& !empty( $_POST["edit_comment"] ) ){
    ...
    $sql = 'UPDATE sample2Table SET name=:name,comment=:comment  WHERE id=:id';
    $stmt = $pdo->prepare($sql);
